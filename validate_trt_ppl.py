...
def validate(args):
    ...
    cuda.memcpy_htod_async(trt_infer.allocations_pp[0][0], np.ascontiguousarray(input), stream_io)
    stream_io.synchronize()

    trt_infer.context.execute_async_v2(trt_infer.allocations_pp[0], stream_c.handle)
    stream_c.synchronize()
    
    cuda.memcpy_dtoh_async(labels, trt_infer.allocations_pp[0][1], stream_io)
    stream_io.synchronize()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if batch_idx == 0:
            cuda.memcpy_htod_async(trt_infer.allocations_pp[0][0], feed, stream_io) # load next
        
        if batch_idx == 1:
            stream_io.synchronize()
            if args.acc:
                target_torch[1] = target
            trt_infer.context.execute_async_v2(trt_infer.allocations_pp[0], stream_c.handle) # compute current
            cuda.memcpy_htod_async(trt_infer.allocations_pp[1][0], feed, stream_io) # load next
            continue
        ...
        if batch_idx > 1 and batch_idx < len(loader) - 2:
            ...
            trt_infer.context.execute_async_v2(trt_infer.allocations_pp[(batch_idx - 1)%2], stream_c.handle) # compute current
            cuda.memcpy_dtoh_async(labels, trt_infer.allocations_pp[batch_idx%2][1], stream_io) # store pre 
            cuda.memcpy_htod_async(trt_infer.allocations_pp[batch_idx%2][0], feed, stream_io) # load next
